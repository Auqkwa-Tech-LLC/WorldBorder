...
public class BorderData {
    ...
    public Location correctedPosition(Location loc, boolean round, boolean flying) {
        ...
        Chunk tChunk = Objects.requireNonNull(loc.getWorld()).getChunkAt(CoordXZ.blockToChunk(ixLoc), CoordXZ.blockToChunk(izLoc));
        if (!tChunk.isLoaded())
            tChunk.load();
    ...
    private boolean isSafeSpot(World world, int X, int Y, int Z, boolean flying) {
        ...
        return (!safeOpenBlocks.contains(below) || Material.WATER.equals(below)) && !painfulBlocks.contains(below);
    }
