...
public class BorderCheckTask implements Runnable {
    ...
    private static Location newLocation(Player player, Location loc, BorderData border, boolean notify) {
        if (Config.Debug()) {
            Config.logWarn((notify ? "Border crossing" : "Check was run") + " in \"" + Objects.requireNonNull(loc.getWorld()).getName() + "\". Border " + border.toString());
        ...
        if (Config.Debug())
            Config.logWarn("New position in world \"" + Objects.requireNonNull(newLoc.getWorld()).getName() + "\" at X: " + Config.coord.format(newLoc.getX()) + " Y: " + Config.coord.format(newLoc.getY()) + " Z: " + Config.coord.format(newLoc.getZ()));
